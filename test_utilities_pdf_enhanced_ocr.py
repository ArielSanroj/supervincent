#!/usr/bin/env python3
"""
Test script for enhanced utilities PDF OCR system
"""
...
from src.core.ocr.utilities_pdf_enhanced_ocr import UtilitiesPDFEnhancedOCR
import json

def test_utilities_pdf_enhanced_ocr():
    """Test the enhanced utilities PDF OCR system"""
    print("🧪 Testing Enhanced Utilities PDF OCR System")
    print("=" * 60)
    
    try:
        # Initialize utilities PDF OCR processor
        utilities_ocr = UtilitiesPDFEnhancedOCR()
        ...
        # Process the utilities PDF invoice
        result = utilities_ocr.process_utilities_pdf_invoice(pdf_path)
        ...
        if result['success']:
            ...
            print(f"  🔍 Source: {utilities_data.get('source', 'Unknown')}")
            print()
            ...
            print(f"  Source: {result['processing_stats']['source']}")
            print()
            ...
            print("-" * 40)
            expected_total = 529369
            actual_total = utilities_data.get('total_amount', 0)
            print(f"  Expected Total: ${expected_total:,.2f}")
            print(f"  Actual Total: ${actual_total:,.2f}")
            print(f"  Match: {'✅' if abs(actual_total - expected_total) < 1000 else '❌'}")
            
            expected_supplier = "EMPRESAS PUBLICAS DE NILO"
            actual_supplier = utilities_data.get('supplier_name', '')
            print(f"  Expected Supplier: {expected_supplier}")
            print(f"  Actual Supplier: {actual_supplier}")
            print(f"  Match: {'✅' if expected_supplier.lower() in actual_supplier.lower() else '❌'}")
            
            expected_invoice = "358157"
            actual_invoice = utilities_data.get('invoice_number', '')
            print(f"  Expected Invoice: {expected_invoice}")
            print(f"  Actual Invoice: {actual_invoice}")
            print(f"  Match: {'✅' if expected_invoice in actual_invoice else '❌'}")
            print()
            
            print("🎉 Utilities PDF processing completed successfully!")
            return True
...
def main():
    """Main test function"""
    print("🚀 Enhanced Utilities PDF OCR System Test")
    print("=" * 60)
    
    # Test utilities PDF OCR
    if not test_utilities_pdf_enhanced_ocr():
        print("❌ Utilities PDF OCR test failed")
        return False
    
    print("=" * 60)
    print("✅ All tests completed successfully!")
    print("\n🎉 The enhanced utilities PDF OCR system is ready!")
    print("\n📋 Key improvements:")
    print("  ✅ PDF text extraction + Ollama OCR")
