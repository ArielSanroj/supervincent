...
class AlegraReports:
    ...
    def get_auxiliary_ledger(self, contact_id: str, start_date: str, end_date: str) -> Optional[Dict]:
        """
        Obtener ledger auxiliar para un contacto específico
        
        Args:
            contact_id: ID del contacto (proveedor/cliente)
            start_date: Fecha de inicio (YYYY-MM-DD)
            end_date: Fecha de fin (YYYY-MM-DD)
        """
        logger = logging.getLogger(__name__)
        logger.info(f"📊 Generando ledger auxiliar para contacto {contact_id}")
        ...
        try:
            response = requests.get(
                f"{self.base_url}/contacts/{contact_id}/ledger",
                params=params,
                headers=headers,
                timeout=30
            )
            ...
            if response.status_code == 200:
                ...
                logger.info(f"✅ Ledger auxiliar generado exitosamente")
                
                # Guardar ledger en archivo
                self._save_report_to_file(ledger_data, f"auxiliary-ledger-{contact_id}", start_date, end_date)
                
                return ledger_data
            else:
                logger.error(f"❌ Error generando ledger auxiliar: {response.status_code}")
                logger.error(f"📝 Respuesta: {response.text}")
                return None
    ...
    def get_accounts(self) -> Optional[List[Dict]]:
        """Obtener lista de cuentas contables"""
        logger = logging.getLogger(__name__)
        logger.info("📊 Obteniendo lista de cuentas contables")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"✅ Se obtuvieron {len(accounts)} cuentas")
                return accounts
            else:
    ...
    def get_contacts(self, contact_type: str = None) -> Optional[List[Dict]]:
        """
        Obtener lista de contactos
        
        Args:
            contact_type: Tipo de contacto ('client', 'provider', None para todos)
        """
        logger = logging.getLogger(__name__)
        logger.info(f"📊 Obteniendo contactos (tipo: {contact_type or 'todos'})")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"✅ Se obtuvieron {len(contacts)} contactos")
                return contacts
            else:
                logger.error(f"❌ Error obteniendo contactos: {response.status_code}")
                return None
...
def main():
    """Función principal para probar reportes"""
    import argparse
    
    parser = argparse.ArgumentParser(description='Generador de reportes contables')
    parser.add_argument('report_type', choices=['general-ledger', 'trial-balance', 'journal', 'auxiliary-ledger'])
    ...
    parser.add_argument('--account-id', help='ID de cuenta específica')
    parser.add_argument('--contact-id', help='ID de contacto para ledger auxiliar')
    ...
    try:
        ...
        if args.report_type == 'auxiliary-ledger':
            if not args.contact_id:
                print("❌ Se requiere --contact-id para ledger auxiliar")
                return
            
            result = reporter.get_auxiliary_ledger(args.contact_id, args.start_date, args.end_date)
        else:
