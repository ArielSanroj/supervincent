#!/usr/bin/env python3
"""
Sistema mejorado de procesamiento de facturas - Versión corregida
Incluye: mapeo de cuentas contables, validación de datos y manejo mejorado de contactos
"""
...
class InvoiceProcessorFixed:
    """Procesador mejorado de facturas con correcciones críticas"""
    ...
    def validate_invoice_data(self, datos_factura: Dict) -> Tuple[bool, List[str]]:
        ...
        required_fields = ['fecha', 'proveedor' if 'proveedor' in datos_factura else 'cliente', 'items', 'total']
    ...
    def get_or_create_contact_improved(self, name: str, contact_type: str = 'client') -> Optional[str]:
        """Obtener o crear contacto con validación mejorada"""
        ...
        try:
            ...
            # Crear nuevo contacto con datos validados
            contact_defaults = self.accounting_config.get('contact_defaults', {}).get(contact_type, {})
            
            payload = {
                'name': name.strip(),
                'type': contact_type,
                'identificationObject': contact_defaults.get('identificationObject', {
            ...
            if 'email' in contact_defaults:
                payload['email'] = contact_defaults['email']
            if 'phone' in contact_defaults:
                payload['phone'] = contact_defaults['phone']
            
            logger.info(f"📝 Creando contacto: {name} (tipo: {contact_type})")
            ...
            if response.status_code == 201:
                ...
                logger.error(f"❌ Error creando contacto: {response.status_code} - {response.text}")
                
                # Intentar usar contacto por defecto si falla la creación
    ...
    def get_or_create_item_improved(self, name: str, price: float, item_type: str = 'product') -> Optional[str]:
        """Obtener o crear item con cuenta contable"""
        headers = self.get_auth_headers()
        ...
        try:
            ...
            logger.info(f"📦 Creando item: {name} con cuenta contable {accounting_account_id}")
    ...
    def create_purchase_bill_improved(self, datos_factura: Dict) -> Optional[Dict]:
        """Crear factura de compra con cuentas contables"""
        ...
        provider_id = self.get_or_create_contact_improved(datos_factura['proveedor'], 'provider')
        if not provider_id:
        ...
        for item in datos_factura['items']:
            item_id = self.get_or_create_item_improved(item['descripcion'], item['precio'], 'product')
            if item_id:
    ...
    def create_sale_invoice_improved(self, datos_factura: Dict) -> Optional[Dict]:
        """Crear factura de venta con cuentas contables"""
        ...
        client_id = self.get_or_create_contact_improved(datos_factura['cliente'], 'client')
        if not client_id:
    ...
    def process_invoice(self, file_path: str) -> Optional[Dict]:
        ...
        # Procesar según tipo
        if tipo == 'compra':
            return self.create_purchase_bill_improved(datos_factura)
        else:
            return self.create_sale_invoice_improved(datos_factura)
...
def main():
    ...
    try:
        processor = InvoiceProcessorFixed()
